...
export async function validate(jobId, jobConfig, mongoOperator, amqpOperator) {
  ...
  const {hostRecord, linkDataHarvesterValidationFilters} = jobConfig;
  const validators = await pumpValidators(linkDataHarvesterValidationFilters);
  const marcHostRecord = new MarcRecord(hostRecord);

  await pumpMessagesFromQueue(marcHostRecord, validators);
  ...
  async function pumpMessagesFromQueue(marcHostRecord, validators) {
    // Get records
    ...
    const linkedValidData = await pumpValidation(validators, marcHostRecord, records);
    ...
    return pumpMessagesFromQueue(marcHostRecord, validators);
  }

  async function pumpValidation(validators, marcHostRecord, records, linkedValidData = []) {
    ...
    // Create linkData
    const newLinkedValidData = valids.map(record => ({hostRecord: marcHostRecord, changes: [...config.changes], record}));

    return pumpValidation(rest, marcHostRecord, records, [...linkedValidData, ...newLinkedValidData]);
